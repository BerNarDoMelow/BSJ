...
public class SaleRowDataGateway {
	...
	public static SaleRowDataGateway load(ResultSet rs) throws RecordNotFoundException{
		try {
			...
			saleRDG.status = rs.getString("status");
			saleRDG.customerId = rs.getInt("customer_id");
			saleRDG.vendedorId = rs.getInt("vendedor_id");
			return saleRDG;
		} catch (SQLException e) {
    ...
    private static final String SQLGetAllByVendedorId = "select * from sale s where s.vendedor_id = ?";

    public static List<SaleRowDataGateway> findAllSalesByVendedor(int vendedorId) throws PersistenceException {
        try(PreparedStatement statement = DataSource.INSTANCE.prepare(SQLGetAllByVendedorId)){
            statement.setInt(1,vendedorId);
            try(ResultSet resultSet = statement.executeQuery()){
    ...
    public static List<SaleRowDataGateway> loadAll(ResultSet rs) throws PersistenceException{
        try{
            List<SaleRowDataGateway> ls = new ArrayList<>();
            while(rs.next()){
                SaleRowDataGateway saleRDG = new SaleRowDataGateway(rs.getInt("customer_id"),rs.getDate("date"));
                saleRDG.total = rs.getDouble("total");
                saleRDG.discount = rs.getDouble("discount_total");
                saleRDG.status = rs.getString("status");
                saleRDG.customerId = rs.getInt("customer_id");
